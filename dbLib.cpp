...
void loadNinjaDB(char* fName, L1List<NinjaInfo_t> &db) {
	...
	while (!dataFile.eof()) {
		...
		ss >> date;
		date[2] = date[5] = '\0';
		tm_date.tm_mday = atoi(&date[0]);
		tm_date.tm_mon = atoi(&date[3]) - 1;
		...
		//-------------------------------------------------- if using .txt is regardless this code
		if (!dataFile.eof()) {
			getline(dataFile, str);
		}
	}
...
void process0Event(L1List<ninjaEvent_t> &eventList) {
	long run = 0;
	long size = eventList.getSize();
	cout << "0 : ";
	while (run < size) {
		if (run == size - 1) {
			cout << eventList[run].code;
			break;
		}
		cout << eventList[run].code << " ";
		run++;
